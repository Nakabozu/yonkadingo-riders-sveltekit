<script lang="ts">
    ...
    const letterLookup = ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", "v", "w", "x", "y", "z"];
...
<div class="background" style={`--row-count: ${rowCount}; --col-count: ${columnCount}`}>
    <h1>
        Gameboard
    </h1>
    ...
    <div class="the-griddy">
        {#each {length: rowCount} as _, i}
            {#if i === 0}
                <div></div><th>{letterLookup[i].toLocaleUpperCase()}</th>
            {:else}
        ...
        {#each $gameboard as row, rowIndex}
            ...
            {#each row as tileData, colIndex}
                {#if tileData}
                    <Tile {...tileData} rowIndex={rowIndex} colIndex={colIndex}/>
                {:else}
                    <Tile isRevealed={false} rowIndex={rowIndex} colIndex={colIndex}/>
                {/if}
            {/each}
        {/each}
    </div>
