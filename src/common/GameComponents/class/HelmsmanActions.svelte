<script lang="ts">
  ...
  $: lastCoord = $helmsmanMovesPerformed.length > 0
    ...
    switch (move) {
      ...
      case "left":
        if (lastCoord.column <= 0) {
          displayWarningMsg("You're already at the leftmost edge of the lake!");
          return;
        }
      ...
      case "right":
        if (lastCoord.column + 1 >= $gameboard[0].length) {
          displayWarningMsg("You're already at the rightmost edge of the lake!");
          return;
        }
        newCoord = { row: lastCoord.row, column: lastCoord.column + 1 };
        break;
    ...
    if((
      $helmsmanMovesPerformed.length === 1 
        ...
        $helmsmanMovesPerformed.length >= 2 
          && isCoordEqual($helmsmanMovesPerformed[$helmsmanMovesPerformed.length - 2], newCoord)
      ))
    {
      console.log("UNDOING A MOVE VIA ARROWS")
      undoMove();
    }
    else if($totalHelmsmanMovesAllowed - $helmsmanMovesPerformed.length <= 0){
